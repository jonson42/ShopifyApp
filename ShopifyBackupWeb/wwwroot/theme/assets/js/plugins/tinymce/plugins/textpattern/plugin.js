...
var textpattern = (function () {
  ...
  var $_ailj4frtjkc9bjox = { getPatterns: getPatterns };
  ...
  var patternFromRng = function (patterns, rng, space) {
    ...
    var endPattern = $_45wf67s1jkc9bjpw.findEndPattern(patterns, text, rng.startOffset, delta);
    ...
    var startPattern = $_45wf67s1jkc9bjpw.findPattern(patterns, patternRng.toString());
  ...
  var applyBlockFormat = function (editor, patterns) {
    ...
    if (textBlockElm) {
      ...
      if (firstTextNode) {
        pattern = $_45wf67s1jkc9bjpw.findPattern(patterns, firstTextNode.data);
        if (!pattern) {
          return;
        }
  ...
  function handleEnter(editor, patterns) {
    ...
    wrappedTextNode = $_91w278ryjkc9bjpf.applyInlineFormatEnter(editor, patterns);
    if (wrappedTextNode) {
    ...
    $_91w278ryjkc9bjpf.applyBlockFormat(editor, patterns);
  }
  function handleInlineKey(editor, patterns) {
    var wrappedTextNode, lastChar, lastCharNode, rng, dom;
    wrappedTextNode = $_91w278ryjkc9bjpf.applyInlineFormatSpace(editor, patterns);
    if (wrappedTextNode) {
      dom = editor.dom;
  ...
  var setup = function (editor, patternsState) {
    ...
    editor.on('keydown', function (e) {
      if (e.keyCode === 13 && !global$2.modifierPressed(e)) {
        $_a40zpkrxjkc9bjp7.handleEnter(editor, patternsState.get());
      }
    }, true);
    editor.on('keyup', function (e) {
      if ($_a40zpkrxjkc9bjp7.checkKeyCode(keyCodes, e)) {
        $_a40zpkrxjkc9bjp7.handleInlineKey(editor, patternsState.get());
      }
    });
    editor.on('keypress', function (e) {
      if ($_a40zpkrxjkc9bjp7.checkCharCode(charCodes, e)) {
  ...
  var $_514q8yrujkc9bjoz = { setup: setup };

  global.add('textpattern', function (editor) {
    var patternsState = Cell($_ailj4frtjkc9bjox.getPatterns(editor.settings));
    $_514q8yrujkc9bjoz.setup(editor, patternsState);
    return $_9sohzgrsjkc9bjou.get(patternsState);
  });
  function Plugin () {
  }
